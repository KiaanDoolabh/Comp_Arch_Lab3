...
// This Vivado version removes the ModelSim testbench and connects the
// pipelined CPU to the Lab 2 AXI memory-controller handshake.
...
// Vivado-facing top level. This replaces the ModelSim testbench/top wrapper
// with the same memory-controller handshake style used in Lab 2.
module top(input  wire        clk,
   ...
   (* mark_debug = "true" *) wire [31:0] InstrF;
   (* mark_debug = "true" *) wire        MemWriteM;
   (* mark_debug = "true" *) wire        MemStrobe;
   (* mark_debug = "true" *) wire        PCReady;

   // The pipelined processor uses BRAM for instruction fetch and the AXI-side
   // memory controller for data memory accesses.
   riscv rv32pipe(.clk(clk),
                  .reset(reset),
                  .PCF(PCF),
                  .InstrF(InstrF),
                  .MemWriteM(MemWriteM),
...
module riscv(input  logic        clk, reset,
   ...
   logic 			 StallFHazard, StallDHazard, FlushDHazard, FlushEHazard;
   logic                         MemWait;
   ...
   controller c(clk, reset,
		...
		MemWriteM, RegWriteM, ResultSrcM,
		RegWriteW, ResultSrcW, MemWait);
   ...
   datapath dp(clk, reset,
	       ...
	       FlushE, ForwardAE, ForwardBE, PCSrcE, ALUControlE,
               ...
               Rs1D, Rs2D, Rs1E, Rs2E, RdE, RdM, RdW, MemWait);

   hazard  hu(Rs1D, Rs2D, Rs1E, Rs2E, RdE, RdM, RdW,
              PCSrcE, ResultSrcEb0, RegWriteM, RegWriteW,
              ForwardAE, ForwardBE, StallFHazard, StallDHazard,
              FlushDHazard, FlushEHazard);

   // AXI/DDR memory is not single-cycle. When a load or store reaches MEM,
   // assert the strobe and freeze the pipeline until MemControl reports ready.
   assign MemStrobe = MemWriteM | ResultSrcM[0];
   assign MemWait = MemStrobe & ~PCReady;
   assign StallF = StallFHazard | MemWait;
   assign StallD = StallDHazard | MemWait;
   assign FlushD = FlushDHazard & ~MemWait;
   assign FlushE = FlushEHazard & ~MemWait;
endmodule
...
module controller(input  logic		 clk, reset,
   ...
   flopenrc #(14) controlregE(clk, reset, FlushE, ~MemWait,
   ...
   flopenr #(4) controlregM(clk, reset, ~MemWait,
                            {RegWriteE, ResultSrcE, MemWriteE},
                            {RegWriteM, ResultSrcM, MemWriteM});
   
   // Writeback stage pipeline control register
   flopenr #(3) controlregW(clk, reset, ~MemWait,
...
module datapath(input logic clk, reset,
   ...
   flopenrc #(178) regE(clk, reset, FlushE, ~MemWait,
   ...
   flopenr #(104) regM(clk, reset, ~MemWait,
   ...
   flopenr #(104) regW(clk, reset, ~MemWait,
...
module imem (input  logic [31:0] a,
   ...
   // Add this .memfile to Vivado as a design source so the BRAM initializes.
   // Change the filename here if you want a different FPGA program image.
   initial
     $readmemh("test_hw2.memfile", RAM);
...
module MemControl(output logic Done,
                  input  logic MStrobe,
   ...
   // Lab 2 memory-control FSM. MStrobe starts a processor memory transaction;
   // Trigger comes back from the AXI/DDR side when the transfer completes.
   ...
   always_comb
     case(state)
       Idle:
         begin
            if (~MStrobe)        nextstate = Idle;
            else if (MemWrite)   nextstate = WriteStart;
            else                 nextstate = ReadStart;
            OutputLogic = 3'b110;
         end
       ReadStart:
         begin
       ...
       ReadMem:
         begin
            nextstate = Trigger ? ReadData : ReadMem;
            OutputLogic = 3'b000;
         end
       ReadData:
         begin
       ...
       WriteMem:
         begin
            nextstate = Trigger ? WriteData : WriteMem;
            OutputLogic = 3'b010;
         end
       WriteData:
         begin
